...
if($home_id == $userid && empty($only)){
    echo $this->Html->script('GetTweet.js');
}
echo $this->Html->script('PostingTweet.js');
...
if($home_id == $userid && $only == null){
    ...
    echo "<div>";
    echo "<textarea id='TweetMessage' placeholder='140字以内で入力してください'></textarea>";
    echo "</div>";
    echo "</div>";
    echo "<div class='tweet_area'>";
    echo "<div id='recent_tweet' class='content'>";
    ...
    echo "</div>";
    echo "<button id='toukou_button' class='submit_button'>投稿する</button>";
    echo "</div>";
    echo "</div>";
        ...
        foreach($search_result as $val){
            ...
            if($val['Tweet']['username'] == $this->Session->read('userid')){
                ...
                if(!empty($only)){
                    ...
                    //自分のホーム
                    $next_url = "http://donatu33.sakura.ne.jp/TwitterModoki/tweets/home/".$this->Session->read('userid').'/';
                }
